...
const _keys = require('../configurations/mongodb')
...
class MongoAPI extends MongoClient {
    ...
    */
    async bulkInsert(collection, dataArray) {
        ...
        try {
            ...
            console.log(`Bulk Insert Unsuccessful \n ${error}`); 
            throw error;
        }
    ...
    * @returns {Object|null}
    */
    async search(collection, filter = null, sortParams = null, suppress_id = false) {
        if (!this.isConnected()) { await this.establishConnection(); }

        try {
            const dbCollection = this.client.db(_keys.DB).collection(collection);
            let foundItem = '';
            if(!sortParams && !suppress_id) {
                foundItem = await dbCollection.find(filter).toArray();
            }
            else if(sortParams) {
                foundItem = await dbCollection.find(filter).sort(sortParams).toArray();
            }
            else if(suppress_id){
                foundItem = await dbCollection.find(filter).project({_id:0, formID:0}).toArray();
            }
